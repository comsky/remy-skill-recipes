...
기존 컨벤션 탐색, 코드-문서 정합성 검증, 브랜치/워크트리 목적 검증,
깔끔한 커밋 구조화, publish 이후 정리 선택을 수행합니다.
...
- 스테이징/커밋 전 브랜치와 워크트리 목적 일치 여부 점검
- 변경 유형별 커밋 구조화: `docs(fix)`, `docs(style)`, `docs(content)`, `docs(sync)`
- 푸시, PR 생성, 머지 후 브랜치/워크트리 정리 선택 요구
...
동시에 오래된 브랜치나 이전 세션의 worktree 변경이 섞이지 않게 하고,
publish 이후 stale 작업 브랜치가 남는 문제를 줄입니다.

---

## 절차 (9-Gate)

- **Gate -1: Branch Context Check** — 현재 브랜치/워크트리가 작업 목적과 맞는지 확인
...
7. **Post-Publish Cleanup Handoff** — 커밋/푸시/PR/머지 후 정리 선택 실행
